...
class AmazonMarketplaceConfig(models.TransientModel):
    ...
    def _is_mkt_created_via_program(self):
    ...
    is_mkt_created_via_program = fields.Boolean('Is MKT created ?', compute='_is_mkt_created_via_program')
    is_european_region = fields.Boolean('Is European Region ?', compute='_compute_region',
                                        default=False)
    is_north_america_region = fields.Boolean('Is North America Region ?', compute='_compute_america_region', default=False)
    ...
    def _compute_region(self):
        is_european = self.seller_id.marketplace_ids.filtered(
            lambda x: x.market_place_id in ['A1PA6795UKMFR9',
                                            'A1RKKUPIHCS9HS',
                                            'A13V1IB3VIYZZH',
                                            'A1F83G8C2ARO7P', 'APJ6JRA9NG5V4',
                                            'A1805IZSGTT6HS','A2NODRKZP88ZB9','A1C3SOZRARQ6R3'])
    ...
    def _onchange_amazon_program(self):
        ...
        if self.amazon_program == 'efn+mci':
            ...
            if seller_country_marketplace:
                self.mci_efn_marketplace_ids = seller_country_marketplace
    ...
    def get_fba_warehouse(self, view_location_id, lot_stock_id, marketplace, unsellable_location, country=False):
        ...
        # partner country
        vals = {'name': self._context.get('warehouse_name') if self._context.get('warehouse_name',
                                                                                 False) else 'FBA %s(%s)' % (
    ...
    def create_unsellable_location(self):
        stock_location_obj = self.env['stock.location']
        unsellable_location = stock_location_obj.search([('name', '=', self.seller_id.name + " Unsellable"),
                                                         ('usage', '=', 'internal'),
                                                         ('company_id', '=', self.seller_id.company_id.id)])
    ...
    def switch_amazon_fulfillment_by(self):
        ...
        if self.amazon_selling == self.seller_id.amazon_selling:
            raise Warning('Selected fulfillment is already set in seller.')
        ...
        if self.amazon_selling != 'FBM':
            ...
            if not self.seller_id.is_european_region:
                ...
                for instance in self.seller_id.instance_ids:
                    ...
                    fba_warehouse_id and fba_warehouse_id.write({'resupply_wh_ids': [warehouse_id]})
                ...
                for marketplace in self.marketplace_ids:
                    ...
                    if self.amz_fba_us_program == 'narf':
                        ...
                        fba_warehouse_id = self.get_fba_warehouse( view_location_id, lot_stock_id, marketplace,
                                                                   unsellable_location, marketplace.country_id)
                    vals.update({'fba_warehouse_id': fba_warehouse_id.id})
            ...
            if self.amazon_program == 'pan_eu':
                ...
                resupply_wh = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                ...
                if self.store_inv_wh_efn:
                    warehouse_id = self.search_fbm_warehouse(self.seller_id.company_id)
                    unsellable_location = self.create_unsellable_location()
                    fba_warehouse = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                                                                              r.is_fba_warehouse == True and
                                                                              r.partner_id.country_id.id == self.store_inv_wh_efn.id)
                ...
                for inv_store_wh in self.store_inv_wh_mci:
                    fba_warehouse = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                                                                              r.is_fba_warehouse == True and
                                                                              r.partner_id.country_id.id == inv_store_wh.id)
                    if not fba_warehouse:
                        fba_warehouse = self.get_fba_warehouse(False, False, False,
                ...
                for marketplace in self.mci_marketplace_ids:
                    ...
                    if not wh_for_mkt:
                        wh_for_mkt = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                                                                               r.is_fba_warehouse == True and r.partner_id.country_id.id == self.seller_id.country_id.id)
                ...
                de_fba_wh = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                                                                      r.is_fba_warehouse == True and
                                                                      r.partner_id.country_id.code == 'DE')
                if not de_fba_wh:
                    de_fba_wh = self.get_fba_warehouse(False, False, False,
                ...
                for store_inv in self.store_inv_wh_cep.filtered(lambda r: r.code in ('CZ', 'PL')):
                    fba_warehouse = self.seller_id.amz_warehouse_ids.filtered(lambda r:
                                                                              r.is_fba_warehouse == True and
                                                                              r.partner_id.country_id.id == store_inv.id)
                    if not fba_warehouse:
        ...
        if not self.amazon_program:
            ...
            for marketplace in self.marketplace_ids:
                ...
                if self.amazon_selling != 'FBM':
                    ...
                    vals.update({'fba_warehouse_id': fba_warehouse_id.id})
                    fba_warehouse_id and fba_warehouse_id.write({'resupply_wh_ids': warehouse.ids})
                if amz_tax:
                    vals.update(amz_tax)
                amazon_instance_obj.create(vals)
            ...
            for marketplace in self.pan_eu_marketplace_ids:
                ...
                if instance_exist:
                    continue
                vals = self.prepare_amazon_marketplace_vals(marketplace, warehouse_id)
                ##Skip Creating FBA Warehouse for NL and SE Marketplace, Use DE Warehosue
            ...
            view_location_id, lot_stock_id = False, False
            warehouse_id = self.search_fbm_warehouse(self.seller_id.company_id)
            fba_warehouse = self.seller_id.mapped('amz_warehouse_ids').filtered(lambda r:
                                                                                r.is_fba_warehouse == True and
                                                                                r.partner_id.country_id.id == self.store_inv_wh_efn.id)
            if not fba_warehouse:
            ...
            if not self.store_inv_wh_mci and not 'GB' in self.mci_marketplace_ids.mapped('country_id').mapped('code'):
                raise Warning('Please select at least one Store Inventory Country')
            # unsellable_location = self.create_unsellable_location()
            view_location_id, lot_stock_id = False, False
            warehouse_id = self.search_fbm_warehouse(self.seller_id.company_id)
            warehouse = self.env['stock.warehouse'].browse(warehouse_id)
            ...
            for marketplace in self.mci_marketplace_ids:
                ...
                if instance_exist:
                    raise Warning(
                        'Instance already exist for %s with given Credential.' % (marketplace.name))
                wh_for_mkt = self.seller_id.mapped('amz_warehouse_ids').filtered(lambda r:
                                                                                 r.is_fba_warehouse == True and
                                                                                 r.partner_id.country_id.id == marketplace.country_id.id)
                if not wh_for_mkt:
                    wh_for_mkt = self.seller_id.mapped('amz_warehouse_ids').filtered(lambda r:
            ...
            if not de_fba_wh:
                ...
                de_fba_wh = self.get_fba_warehouse(view_location_id, lot_stock_id, False, unsellable_location_ept,
                                                   de_country)
    ...
    def create_adjustment_config(self, seller, stock_reason_group, location_id, is_send_email=False,
        ...
        amz_stock_adjustment_config.create(
            {'seller_id': seller, 'group_id': stock_reason_group and stock_reason_group.id,
