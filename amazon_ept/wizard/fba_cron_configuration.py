...
class FbaCronConfiguration(models.TransientModel):
    ...
    #inbound cron
    amz_auto_import_inboud_shipment_status = fields.Boolean(
    ...
    def update_amz_inbound_shipment_status(self,amz_seller):
        ...
        if amz_inbound_status_cron:
            self.amz_auto_import_inboud_shipment_status=amz_inbound_status_cron.active or False
            self.amz_shipment_status_import_next_execution=amz_inbound_status_cron.nextcall or False
            self.amz_shipment_status_import_interval_number=amz_inbound_status_cron\
                .interval_number or False
            self.amz_shipment_status_import_interval_type=amz_inbound_status_cron.interval_type or False
    ...
    def update_amz_fba_stock_auto_import_cron_field(self, amz_seller):
        ...
        if amz_check_stock_report_cron_exist:
            ...
            self.fba_stock_adj_report_user_id = amz_check_stock_report_cron_exist.user_id.id or False
    ...
    def save_cron_configuration(self):
        ...
        vals['amz_auto_import_inboud_shipment_status']=self.amz_auto_import_inboud_shipment_status or False
        ...
        if self.amz_auto_import_shipment_report and self.amazon_selling not in ['FBM']:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_process_amazon_fba_shipment_report_seller_%d' % (
                    amazon_seller.id),
                raise_if_not_found=False)
            process_next_execution = self.amz_ship_report_import_next_execution + relativedelta(minutes=10)
            vals = {
                'active': True,
                'interval_number': self.amz_ship_report_import_interval_number,
                'interval_type': self.amz_ship_report_import_interval_type,
                ...
                'user_id': self.amz_ship_report_import_user_id.id,
                'code': "model.auto_process_shipment_report({'seller_id':%d, 'is_auto_process': True})" % (amazon_seller.id),
        ...
        if self.amz_auto_import_return_report and self.amazon_selling not in ['FBM']:
            cron_exist = self.env.ref(
                'amazon_ept.ir_cron_auto_process_customer_return_report_seller_%d' % (
                    amazon_seller.id),
                raise_if_not_found=False)
            process_next_execution = self.amz_return_report_import_next_execution + relativedelta(minutes=10)
            vals = {
                'active': True,
                'interval_number': self.amz_return_report_import_interval_number,
                'interval_type': self.amz_return_report_import_interval_type,
                'nextcall': process_next_execution,
                'user_id': self.amz_return_report_import_user_id.id,
                'code': "model.auto_process_return_order_report({'seller_id':%d, 'is_auto_process': True})" % (amazon_seller.id),
                'amazon_seller_cron_id': amazon_seller.id}
            if cron_exist:
                cron_exist.write(vals)
            else:
    ...
    def setup_amz_seller_live_stock_cron(self, amazon_seller, auto_import, interval_type, interval_number,
        ...
        for cron_xml_id in cron_xml_ids:
            if auto_import and self.amazon_selling not in ['FBM']:
                ...
                uk_cron_exist = self.env.ref(module + '.' + 'ir_cron_import_stock_from_amazon_uk_fba_live_report_uk_seller_%d' % (amazon_seller.id),
                                          raise_if_not_found=False)
                if uk_cron_exist:
    ...
    def setup_inbound_shipment_status_cron(self,seller):
        if self.amz_auto_import_inboud_shipment_status:
            cron_exist = self.env.ref(
