...
class AmazonProductMapping(models.TransientModel):
    ...
    amazon_selling = fields.Selection([('FBA', 'FBA'),
                                      ...
                                      default='FBM',
                                      help='Use to Prepare file for Amazon selling fullfilment')
    def prepare_product_for_export(self):
        ...
        return {
            ...
            'url': '/web/binary/download_document'
                   '?model=amazon.prepare.export.product.ept'
                   '&field=datas&id=%s&filename=amazon_product_export.csv' % (
                       self.id),
            'target': 'self',
        }
