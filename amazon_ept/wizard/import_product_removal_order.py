...
class ImportProductRemovalOrder(models.TransientModel):
    ...
    def wizard_view(self):
        ...
        :return: This Method return wizard.
                 If removal view exist then return removal order wizard If now then raise warning.
        """
        ...
        if removal_order_view:
            ...
            raise Warning("Removal order view not created")
    ...
    def validate_process(self):
        ...
        If not order state draft raise warning,Check order,removal plan and removal order file.
        ...
        for order in removal_order_obj.browse(self._context.get('active_ids')):
            if order.state != "draft":
                raise Warning(_('Unable to process..! You can process with only draft shipment plan!.'))
            orders.append(order)

        if len(orders) > 1:
            raise Warning(_('Unable to process..! You can process only one shipment plan at a time!.'))

        removal_plan = orders[0]
        if not removal_plan:
            raise Warning(_('Unable to process..! Shipment Plan is not found!!!.'))

        if not self.choose_file:
            raise Warning(_('Unable to process..! Please select file to process...'))
        ...
        if field_name and self.validate_fields(field_name):
            ...
            for data in product_data:
                ...
                try:
                    ...
                    raise Warning(_('Unable to process ..!' +
