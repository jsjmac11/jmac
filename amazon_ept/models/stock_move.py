...
class StockMove(models.Model):
    ...
    amazon_shipment_id = fields.Char(size=120, string='Amazon Shipment ID', default=False, copy=False,
                                     help="Shipment Item ID provided by Amazon when we integrate "
                                          "shipment report from Amazon")

    amazon_shipment_item_id = fields.Char(size=120, string='Amazon Shipment Item ID', default=False, copy=False,
                                          help="Shipment Item ID provided by Amazon "
                                               "when we integrate shipment report from Amazon")
    tracking_number = fields.Char(string="Amazon Shipment Tracking No", size=120, default=False, copy=False)
    ...
    fba_returned_date = fields.Datetime("Return Date", help="When return processed in the FBA warehouse.")
